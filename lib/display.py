...
def display_snow(snow: Snow, snow_color: str, road_color: str) -> None:
    ...
    log.info(
        f"Snow amount for district '{district.name}': {snow_amount} edges"
    )
...
def _main_snow(
    ...
    edge_colors = [
        (
            snow_color
            if (u, v, k) in snow_tuples or (v, u, k) in snow_tuples
            else road_color
        )
...
def display_image_route(
    district: District, route: Route, route_color: str, road_color: str
) -> None:
    """
    ...
    log.info(
        f"Route amount for district '{district.name}': {amount} edges"
    )
